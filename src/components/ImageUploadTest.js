...
export default function ImageUploadTest() {
  ...
  const handleGalleryFilesChange = async (e) => {
    ...
    if (files.length > 0) {
      try {
        ...
        addResult(`✅ ${previews.length} previews created successfully`, 'success');
      } catch (err) {
  ...
  const testProfileUpload = async () => {
    ...
    try {
      ...
      const result = await uploadProfileImage(selectedProfileFile, tempMemberId);
  ...
  const testGalleryUpload = async () => {
    ...
    try {
      ...
      addResult(`📤 Starting gallery upload (${selectedGalleryFiles.length} images)...`, 'info');
      
      const results = await uploadGalleryImages(selectedGalleryFiles, tempMemberId);
      
      setUploadProgress(prev => ({ ...prev, gallery: 100 }));
      addResult(`✅ Gallery upload successful! ${results.length} images uploaded`, 'success');
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-700">Profile Image Test</h3>
        ...
        {/* Gallery Images Test */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-700">Gallery Images Test</h3>
      ...
      <div className="mt-8">
        ...
        <div className="bg-gray-50 rounded-lg p-4 max-h-96 overflow-y-auto">
          {results.length === 0 ? (
            <p className="text-gray-500 text-sm">No test results yet. Select images and run tests.</p>
          ) : (
