...
const HumanAvatar = ({ gender = 'male', size = 160, seed = 0 }) => {
  ...
  const variants = {
    ...
    female: {
      skinTones: ['#FDBCB4', '#F1C27D', '#E0AC69', '#D29962', '#C68642'],
      hairColors: ['#2C1B18', '#8B4513', '#D2691E', '#DEB887', '#F4A460', '#FFD700'],
      hairStyles: [
        // Long straight
  ...
  const hairColor = config.hairColors[Math.abs(seed * 2) % config.hairColors.length];
  const hairStyle = config.hairStyles[Math.abs(seed * 3) % config.hairStyles.length];
  const shirtColor = config.shirtColors[Math.abs(seed * 4) % config.shirtColors.length];
  ...
  return (
    ...
    >
      ...
      {/* Nose */}
      <ellipse cx="50" cy="48" rx="1.5" ry="2" fill="none" stroke={skinTone} strokeWidth="0.5" opacity="0.3" />
      
      {/* Mouth */}
      <path d="M46,54 Q50,57 54,54" stroke="#d63384" strokeWidth="1.5" fill="none" strokeLinecap="round" />
      
      {/* Eyebrows */}
      <path d="M39,37 Q42,35 45,37" stroke={hairColor} strokeWidth="1" fill="none" strokeLinecap="round" />
      <path d="M55,37 Q58,35 61,37" stroke={hairColor} strokeWidth="1" fill="none" strokeLinecap="round" />
