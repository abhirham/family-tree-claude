...
export default function AssignAdminModal({ isOpen, onClose, selectedMember, onSuccess }) {
    ...
    try {
      ...
      if (onSuccess) {
        onSuccess({
          ...
          adminEmail: adminType === 'new' ? newAdminData.email : existingUsers.find(u => u.id === selectedUser)?.email
        });
      }
      ...
      <div className="relative z-10 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="bg-white rounded-lg shadow-airbnb border border-gray-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div>
              <h2 className="text-2xl font-semibold text-gray-800">Assign Branch Admin</h2>
              <p className="text-sm text-gray-600 mt-1">
                Create a branch starting from <span className="font-medium text-airbnb-rausch">{selectedMember.name}</span>
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6">
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      onChange={(e) => setNewAdminData(prev => ({ ...prev, tempPassword: e.target.value }))}
                      required
                      disabled={loading}
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    The new admin will be forced to change this password on first login
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div>
                ...
                <AutoComplete
                  ...
                  renderOption={(user, isHighlighted) => (
                    <div className={`flex items-center justify-between ${isHighlighted ? 'text-airbnb-rausch' : 'text-gray-900'}`}>
                      <span>{user.email}</span>
                      <span className="text-xs text-gray-500 capitalize">{user.role || 'branch_admin'}</span>
                    </div>
                  )}
                />
              </div>
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-blue-700">
                    <ul className="list-disc list-inside space-y-1">
                      <li>A new branch will be created starting from {selectedMember.name}</li>
                      <li>All descendants of {selectedMember.name} will be assigned to this branch</li>
                      <li>The assigned admin will only be able to manage members in this branch</li>
                      <li>{adminType === 'new' ? 'The new admin will receive login credentials and must change their password' : 'The existing user will gain access to this new branch'}</li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-6">
              ...
              <button
                type="submit"
                disabled={loading || (adminType === 'new' && (!newAdminData.email || !newAdminData.tempPassword)) || (adminType === 'existing' && !selectedUser)}
              ...
              >
                {loading ? (
                  ...
                  adminType === 'new' ? 'Create & Assign' : 'Assign'
                )}
              </button>
            </div>
          </form>
