...
function generateDummyParentId() {
  return (
    "dummy_parent_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9)
  );
}
...
export async function addFamilyMemberWithRelationships(formData) {
  ...
  try {
    ...
    if (!linkedMemberId || !relationshipType) {
      ...
      console.log(
        "🔍 Debug: First user detected, setting parentIds=[] and root=true",
      );
    }
    ...
    if (linkedMemberId && relationshipType) {
      ...
      switch (relationshipType) {
        case "parent":
          ...
          const existingParents = allMembers
            .filter((member) => member.id === linkedMemberId)
          ...
          if (existingParents.length > 0) {
            ...
            const childrenToUpdate = allMembers.filter(
              (child) =>
            ...
            console.log(
              `🔄 Debug: Added ${memberData.name} as spouse to ${existingParent.name}`,
            );
          } else {
            ...
            if (
              linkedMemberData.parentIds &&
              linkedMemberData.parentIds.length > 0
            ) {
              ...
              const siblings = allMembers.filter(
                (member) =>
                  ...
                  member.parentIds &&
                  member.parentIds.some((pid) =>
                    linkedMemberData.parentIds.includes(pid),
                  ),
              );
              allSiblings.push(...siblings);
            } else {
              // No parents - find other root members without parents to group together
              const otherRootMembers = allMembers.filter(
                (member) =>
                  member.root === true &&
                  member.id !== linkedMemberId &&
            ...
            console.log(
              "🔍 Debug: Setting up new parent for children:",
              allSiblings.map((s) => s.name),
            );
        ...
        case "spouse":
          ...
          const shouldTakeOverRoot =
            memberData.gender === "male" && linkedMember.root === true;
          ...
          console.log(
            "🔧 Bug Fix: Adding spouse to parent with existing children",
          );
          const familyMembers = await getAllFamilyMembers();
          const existingChildren = familyMembers.filter(
            (member) =>
              member.parentIds && member.parentIds.includes(linkedMemberId),
          );

          console.log(
            `🔧 Found ${existingChildren.length} existing children to update:`,
          ...
          for (const child of existingChildren) {
            ...
            if (!updatedParentIds.includes(newMemberId)) {
              ...
              console.log(
                `🔧 Updated ${child.name} to have both parents:`,
                updatedParentIds,
              );
            }
          }
          ...
          console.log(
            "✅ Bug Fix: All existing children now have both parents",
          );
          break;
        ...
        case "sibling":
          ...
          if (linkedMemberForSibling.root === true) {
            ...
            if (
              linkedMemberForSibling.parentIds &&
              linkedMemberForSibling.parentIds.length > 0
            ) {
              ...
              console.log(
                "🔍 Debug: Creating dummy parent ID for root siblings:",
                dummyParentId,
              );
            ...
            const dummyParentId = generateDummyParentId();
            console.log(
              "🔍 Debug: Creating dummy parent ID for non-root siblings:",
              dummyParentId,
            );
  ...
  try {
    ...
    if (imagesToDelete.length > 0) {
      try {
        ...
        console.warn(
          "Warning: Could not delete some images for member",
          id,
          ":",
          imageError,
        );
...
export async function testRelationshipFlow(parentName, childName) {
  try {
    ...
    console.log(
      `🔍 Testing relationship between ${parent.name} (parent) and ${child.name} (child)`,
    );
    console.log(`👨‍👩‍👧‍👦 Child's parentIds:`, child.parentIds || []);

    if (child.parentIds && child.parentIds.includes(parent.id)) {
      console.log(
        `✅ SUCCESS: ${parent.name}'s ID (${parent.id}) is stored in ${child.name}'s parentIds`,
      );
      return true;
    } else {
      console.log(
        `❌ FAILED: ${parent.name}'s ID is NOT in ${child.name}'s parentIds`,
      );
      return false;
    }
  } catch (error) {
  ...
  try {
    ...
    function findDescendants(memberId) {
      ...
      const children = allMembers.filter(
        (member) => member.parentIds && member.parentIds.includes(memberId),
      );
...
export async function createSuperAdmin(email, password) {
  ...
  try {
    ...
    try {
      ...
      if (authError.code === "auth/email-already-in-use") {
        ...
        try {
          ...
          throw new Error(
            "Email exists in Firebase Auth but password is incorrect. Please check the password or delete the existing user in Firebase Auth console.",
          );
        }
      } else {
        throw authError;
    ...
    if (existingProfile) {
      console.log(
        "User profile already exists in Firestore with role:",
    ...
    if (
      wasSignedIn &&
      previousUser &&
      auth.currentUser?.uid !== previousUser.uid
    ) {
      try {
        await signOut(auth);
      } catch (signOutError) {
